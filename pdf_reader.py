...
if opcao.isdigit():
    ...
    if 1 <= opcao <= len(lista_tabelas):
        ...
        # Extrair opções disponíveis da tabela escolhida
        opcoes_disponiveis = [linha[0] for linha in linhas]
...
=======
        ...
        if escolha_opcao.isdigit():
            ...
            if 0 <= escolha_opcao < len(opcoes_disponiveis):
                ...
                try:
                    coeficiente_aprov = tabela_escolhida.loc[tabela_escolhida['USOS PERMITIDOSHABITACIONAIS'] == opcao_escolhida, 'COEF.'].values[0]
                    taxa_ocupacao = tabela_escolhida.loc[tabela_escolhida['USOS PERMITIDOSHABITACIONAIS'] == opcao_escolhida, 'TAXA DE OCUPAÇÃO'].values[0]
                    ca = coeficiente_aprov * area_terreno
                    to = taxa_ocupacao * area_terreno
